...
def algorithm(env, K=4, time_limit=20*60, print_info=False):
    ...
    rt = 0
    print("Instance N {} started at {}".format(env.inst_num, now))
    while N_set and time.time() - start_time < time_limit:
        # take new node
        tau = N_set.pop(0)
        # master problem
        start_mp = time.time()
        theta, x, y = scenario_fun_build(K, tau, env, return_model=False)
        ...
        if zeta <= 1e-04:
            if print_info:
                now = datetime.now().time()
                print("Instance N {}: ROBUST at iteration {} ({}) (time {})   :theta = {},    Xi{},   prune count = {}".format(
        ...
        if time.time() - start_time - prev_save_time > 10*60:
            ...
            with open("Results/Decisions/tmp_results_cb_normal_K{}_N{}_inst{}.pickle".format(K, env.N, env.inst_num), "wb") as handle:
    ...
    now = datetime.now().time()
    print("Instance N {} completed at {}, solved in {} minutes".format(env.inst_num, now, runtime/60))
    results = {"theta": theta_i, "x": x_i, "y": y_i, "tau": tau_i, "inc_thetas": inc_thetas, "inc_x": inc_x, "inc_y": inc_y, "inc_tau": inc_tau,
            "runtime": time.time() - start_time, "tot_nodes": cum_tot_nodes, "num_nodes_curr": inc_tot_nodes, "mp_time": mp_time, "sp_time": sp_time}

    with open("Results/Decisions/final_results_cb_normal_K{}_N{}_inst{}.pickle".format(K, env.N, env.inst_num), "wb") as handle:
